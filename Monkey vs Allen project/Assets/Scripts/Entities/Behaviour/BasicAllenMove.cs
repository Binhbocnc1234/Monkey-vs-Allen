...
public class BasicAllenMove : Move {
    ...
    public override void Initialize() {
        ...
        groundY = startPos.y;
        pauseTimer = new Timer(pauseBetweenHops);
        pauseTimer.Reset(); // start initial pause
        jumping = false;
    }

    protected override void Update() { //pauseTimer null reference vi initialize() chua duoc goi
        // waiting (pause) state
        if (!jumping) {
            ...
            jumping = true;
            jumpTimer = 0f;
            startPos = transform.position; //startPos become starting point for new jump
        }
        ...
        if (t >= 1f) { //The moment Allen landed on the ground was also the time parabolic path is ended
            transform.position = new Vector2(startPos.x + hopDistance * -1, groundY);
            jumping = false;
        ...
        float x = startPos.x + hopDistance * -1 * t;
